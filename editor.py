...
def click_save_button(event):  # 保存ボタンをクリック時の動作
    save_file = open('test.txt', 'w')  # テキストファイルを開く
...
if __name__ == '__main__':
    ...
    frame = wx.Frame(None, -1, 'テキスト入力＆保存', size=(1280, 720))  # フレーム作成

    panel = wx.Panel(frame, -1)  # ボタンやテキストボックスを配置するためのパネルを作成
    save_button = wx.Button(panel, -1, pos=(10, 10), label='保存')  # 保存ボタンの作成

    text = wx.TextCtrl(panel, -1, pos=(10, 50), size=(1200, 700), style=wx.TE_MULTILINE)  # テキストボックスの作成
